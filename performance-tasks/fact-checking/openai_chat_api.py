...
class DialogueBot(ChatBot):
    ...
    def return_multiturn_response(self, 
                        formatted_input: Iterable) -> tuple:
        """
        Return the bot's response to the user's input in multiturn prompting setting. Assumes input is already formatted.
        """

        messages = self.system_prompt + self.history + formatted_input
